...
public class EmpMenu {
		...
		//WTF
		private SLList<EmployeePart> list = new SLList<EmployeePart>();
		...
		while (inFile.hasNext() )
		{
			...
			EmployeePart emp = new EmployeePart (empID, fName, lName, salary);
			list.add(emp);
	...
	public void printEmpPart() {// CHOICE 1
		...
		public void addEmpPart() { // CHOICE 2
			...
			System.out.println("PLease enter the salary as a double. ");
				...
				EmployeePart part = new EmployeePart (newID, newFName, newLName, newSalary);
				...
				public void findEmpPart(){
					...
					EmployeePart search = new EmployeePart(empId);
					int index= list.find(search);
					
					// GOOD UNTIL THIS POINT
						if(index >= 0)
						{
							EmployeePart p = list.get(index);
							System.out.printf("%-10s %-10s %-10s %-10s \n", p.getEmpID() 
									, p.getfName() 
				...
				public void removeEmpPart(){
					...
					EmployeePart remove = new EmployeePart(empIdToRemove);
